...
then
	[ -d /usr/local/etc/raddb ] && echo '==> WARNING: /usr/local/etc/raddb already exists'
	[ -f /usr/local/etc/radsecproxy.conf ] && echo '==> WARNING: /usr/local/etc/radsecproxy.conf already exists'
...
pkg install -y freeradius3 radsecproxy
...
mkdir -p /usr/local/etc/radsecproxy/certs/
cp radsecproxy/certs/ca.pem radsecproxy/certs/server.key radsecproxy/certs/server.pem /usr/local/etc/radsecproxy/certs/
sysrc radsecproxy_enable=YES

service start radiusd
service start radsecproxy
